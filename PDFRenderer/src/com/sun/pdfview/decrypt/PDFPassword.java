/*
 * File: PDFPassword.java
 * Version: 1.0
 * Initial Creation: May 6, 2010 6:26:53 PM
 *
...
public class PDFPassword
{
     ...
     */
    Vector getPasswordBytes(boolean unicodeConversion)
    {
        // TODO - handle unicodeConversion when we support version 5
        if (this.passwordBytes != null || this.passwordString == null)
        {
        	return PDFUtil.singletonVector(this.passwordBytes);
        }
        else
        {
            if (isAlphaNum7BitString(this.passwordString))
            {
                // there's no reason that this string would get encoded
                // in any other way
                return PDFUtil.singletonVector(PDFStringUtil.asBytes(passwordString));
            }
            else
            {
     ...
     */
    private final static PasswordByteGenerator[] PASSWORD_BYTE_GENERATORS = new PasswordByteGenerator[]
    {
        ...
        new PDFDocEncodingByteGenerator(new Byte((byte)0)),
        // replace undefined chars with ?
        new PDFDocEncodingByteGenerator(new Byte((byte)'?')),
        ...
        new IdentityEncodingByteGenerator(new Byte((byte)0)),
        // replace 2-byte chars with ?
        new IdentityEncodingByteGenerator(new Byte((byte)'?'))
    };
     ...
     */
    private static Vector generatePossiblePasswordBytes(String passwordString)
    {
        final Vector possibilties = new Vector();
        int genLen = PASSWORD_BYTE_GENERATORS.length;
        for(int g = 0; g < genLen; g++)
        {
        	final PasswordByteGenerator generator = PASSWORD_BYTE_GENERATORS[g];
            byte[] generated = generator.generateBytes(passwordString);
            // avoid duplicates
            boolean alreadyGenerated = false;
            int posLen = possibilties.size();
            for (int i = 0; !alreadyGenerated && i < posLen; ++i)
            {
                if (Arrays.equals((byte[])possibilties.elementAt(i), generated))
                {
            ...
            if (!alreadyGenerated)
            {
                possibilties.addElement(generated);
            }
        }
        return possibilties;
    }
    ...
    private boolean isAlphaNum7BitString(String string)
    {
        for (int i = 0; i < string.length(); ++i)
        {
            ...
            if (c >= 127 || !PDFUtil.Character_isLetterOrDigit(c))
            {
                return false;
            }
        }
    ...
    private static abstract class CharsetEncoderGenerator implements PasswordByteGenerator
    {
        ...
        public byte[] generateBytes(String password)
        {
            ...
            {
                encoder.replaceWith(new byte[]{replacementByte.byteValue()});
                encoder.onUnmappableCharacter(CharsetEncoder.ERROR_ACTION_REPLACE);
            }
            else
            {
                encoder.onUnmappableCharacter(CharsetEncoder.ERROR_ACTION_IGNORE);
            }
            try
            {
                final ByteBuffer b = encoder.encode(PDFUtil.wrapString(password));
