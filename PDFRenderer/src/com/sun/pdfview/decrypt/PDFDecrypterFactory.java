/*
 * File: PDFDecrypterFactory.java
 * Version: 1.0
 * Initial Creation: May 6, 2010 10:30:35 PM
 *
...
public class PDFDecrypterFactory
{
     ...
     *  was not able to 
     */
    public static PDFDecrypter createDecryptor(PDFObject encryptDict, PDFObject documentId, PDFPassword password) throws IOException, 
    	EncryptionUnsupportedByPlatformException, EncryptionUnsupportedByProductException, PDFAuthenticationFailureException
    {
        ...
        else
        {
            ...
            if (filter != null && "Standard".equals(filter.getStringValue()))
            {
                ...
                if (v == 1 || v == 2)
                {
                    final PDFObject lengthObj = encryptDict.getDictRef("Length");
                    final Integer length = lengthObj != null ? new Integer(lengthObj.getIntValue()) : null;
                    return createStandardDecrypter(encryptDict, documentId, password, length, false, StandardDecrypter.EncryptionAlgorithm.RC4);
                }
                else if (v == 4)
                {
                    return createCryptFilterDecrypter(encryptDict, documentId, password, v);
                }
                else
                {
     ...
     *  not currently support the specified encryption
     */
    private static PDFDecrypter createCryptFilterDecrypter(PDFObject encryptDict, PDFObject documentId, PDFPassword password, int v) 
    	throws PDFAuthenticationFailureException, IOException, EncryptionUnsupportedByPlatformException, EncryptionUnsupportedByProductException
    {
        PDFUtil.assert(v >= 4, "v >= 4", "crypt filter decrypter not supported for standard encryption prior to version 4");
        ...
        final PDFObject encryptMetadataObj = encryptDict.getDictRef("EncryptMetadata");
        if (encryptMetadataObj != null && encryptMetadataObj.getType() == PDFObject.BOOLEAN)
        {
        ...
        final Hashtable cfDecrypters = new Hashtable();
        ...
        {
            final String cfName = (String)cfNameIt.nextElement();
            ...
            final Integer length = lengthObj != null ? new Integer(lengthObj.getIntValue() * 8) : null;
            ...
            final String cfm = cfmObj != null ? cfmObj.getStringValue() : "None";
            ...
            {
                cfDecrypter = createStandardDecrypter(encryptDict, documentId, password, length, encryptMetadata, StandardDecrypter.EncryptionAlgorithm.RC4);
            }
            else if ("AESV2".equals(cfm))
            {
                cfDecrypter = createStandardDecrypter(encryptDict, documentId, password, length, encryptMetadata, StandardDecrypter.EncryptionAlgorithm.AESV2);
            }
            else
            {
        ...
        final String defaultStreamFilter = stmFObj != null ? stmFObj.getStringValue() : CF_IDENTITY;
        
        PDFObject strFObj = encryptDict.getDictRef("StrF");
        final String defaultStringFilter = strFObj != null ? strFObj.getStringValue() : CF_IDENTITY;
     ...
     *
     */
    private static PDFDecrypter createStandardDecrypter(PDFObject encryptDict, PDFObject documentId, PDFPassword password, Integer keyLength, boolean encryptMetadata, 
    		int encryptionAlgorithm) throws PDFAuthenticationFailureException, IOException, EncryptionUnsupportedByPlatformException, 
    		EncryptionUnsupportedByProductException
    {
        if (keyLength == null)
        {
            keyLength = new Integer(DEFAULT_KEY_LENGTH);
        }
