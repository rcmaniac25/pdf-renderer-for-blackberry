...
/*
 * File: PNGPredictor.java
 * Version: 1.3
 * Initial Creation: May 12, 2010 4:44:00 PM
 *
...
public class PNGPredictor extends Predictor
{
    ...
    public ByteBuffer unpredict(ByteBuffer imageData) throws IOException
    {
        ...
        while(imageData.remaining() >= rowSize + 1)
        {
            ...
            rows.addElement(curLine);
            prevLine = curLine;
        }
        
        // turn into byte array
        ByteBuffer outBuf = ByteBuffer.allocateDirect(rows.size() * rowSize);
        for (Enumeration i = rows.elements(); i.hasMoreElements();)
        {
            outBuf.put((byte[])i.nextElement());
        }
