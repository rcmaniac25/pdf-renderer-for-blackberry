...
/*
 * File: FunctionType0.java
 * Version: 1.3
 * Initial Creation: May 10, 2010 5:25:32 PM
 *
...
public class FunctionType0 extends PDFFunction
{
    ...
    protected void parse(PDFObject obj) throws IOException
    {
        ...
        int len;
        int[] size = new int[len = sizeAry.length];
        for (int i = 0; i < len; i++)
        {
        ...
        if (decodeObj != null)
        {
            ...
            float[] decode = new float[len = decodeAry.length];
            for (int i = 0; i < len; i++)
            {
...
//#else
        	outputs[i + outputOffset] = interpolate(outputs[i + outputOffset], 0, (float)littlecms.internal.helper.Utility.pow(2, getBitsPerSample()) - 1, getDecode(2 * i), getDecode((2 * i) + 1));
//#endif
        }
    }
    ...
    private int[][] readSamples(ByteBuffer buf)
    {
        ...
        int len2;
        int[][] samples = new int[size][len2 = getNumOutputs()];
    ...
    private float multilinearInterpolate(float[] encoded, int od)
    {
    	...
    	int len;
        float[] dists = new float[len = encoded.length];
    ...
    {
    	int len;
        int[] controls = new int[len = encoded.length];
