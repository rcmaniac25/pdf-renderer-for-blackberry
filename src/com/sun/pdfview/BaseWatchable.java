/*
 * File: BaseWatchable.java
 * Version: 1.5
 * Initial Creation: May 7, 2010 10:37:41 PM
 *
...
public abstract class BaseWatchable implements Watchable, Runnable
{
            ...
            while (!isFinished() && getStatus() != Watchable.STOPPED)
            {
                if (isExecutable())
                {
                    ...
                    try
                    {
                        ...
                        while ((getStatus() == Watchable.RUNNING) && (gate == null || !gate.iterate()))
                        {
    ...
    {
        return ((status == Watchable.PAUSED || status == Watchable.RUNNING) && (gate == null || !gate.stop()));
    }
